...
def test_apply_is_idempotent_and_restore_round_trips(tmp_path):
    ...
    assert "get_tensor_model_parallel_rank() != 0" in patched
    assert "get_tensor_model_parallel_rank" in patched
...
def test_apply_upgrades_v1_patch_in_place(tmp_path):
    ...
    assert "get_tensor_model_parallel_rank() != 0" in upgraded


def test_apply_upgrades_v2_patch_in_place(tmp_path):
    target = tmp_path / "example_hidden_states_connector.py"
    previous = SOURCE.replace(ORIGINAL, PREVIOUS_REPLACEMENT, 1).replace(
        IMPORT_ANCHOR, PREVIOUS_IMPORT_REPLACEMENT, 1
    )
    assert previous.count(PREVIOUS_PATCH_MARKER) == 1
    target.write_text(previous, encoding="utf-8")

    assert apply(target) == 0
    upgraded = target.read_text(encoding="utf-8")
    assert upgraded.count(PATCH_MARKER) == 1
    assert PREVIOUS_PATCH_MARKER not in upgraded
    assert "get_tensor_model_parallel_rank() != 0" in upgraded
